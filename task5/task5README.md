...
部分	行号范围	核心功能（极简版）
导入模块	1-24	导入库，缺 thop 自动安装
日志配置	27-40	日志输出到控制台 + train.log
数据增强类	43-78	同步处理图片和标注（框 / 掩码）
数据集下载	81-109	下载行人数据集，有则跳过
自定义数据集类	112-157	加载图片 / 掩码，整理成模型能用的格式
变换函数	160-164	训练 / 验证用不同数据增强
构建 Mask R-CNN 模型	167-194	建模型，改检测 / 掩码头，可冻骨干
训练验证函数	197-247	训练一轮模型，验证算 IoU
推理可视化函数	250-290	图片推理，画框 / 掩码，保存展示
主函数	293-398	训练 / 推理入口，训模型 / 可视化
程序入口	401-406	执行推理模式

(Mask R-CNN 有两个核心 “部件”（叫 “头”），需要改成适配 “只识别行人 + 背景” 的版本：
检测头：负责找图片里行人的边界框，原模型能识别 80 类，这里只需要识别 2 类（背景 + 行人），所以替换这个 “头”，让它只输出行人的框；
掩码头：负责给行人画掩码（把行人区域标出来），同样替换成适配 2 类的版本，只给行人画掩码)
(“骨干” 是模型里负责提取图片基础特征的部分（比如 resnet50），“冻” 就是可选是否冻结这部分的权重：
冻结：训练时骨干网络的参数不更新，只用预训练好的特征，好处是少训练参数、防止小样本数据过拟合；
不冻结：训练时骨干网络的参数也能跟着更新，适配性更好，但训练更慢、易过拟合)
(验证算 IoU（248-280 行）
训练完一轮后，用模型没学过的验证集图片测效果，不算错误、不调参数，只算模型猜的行人框和真实框的重叠率（IoU）)
(IoU 是 Intersection over Union 的缩写，中文叫「交并比」，核心就是衡量模型猜的行人框和真实行人框的重叠程度，数值在 0~1 之间)
